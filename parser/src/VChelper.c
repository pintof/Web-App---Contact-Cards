...
DateTime *parseDateTime(char *dt)
{
    ...
    if (zIndex == -1 && tIndex == -1)
    {
        if(dIndex == -1){
            ...
            DateTime *dt2 = newDateTime(isUTC, isText, dt, time, "");
            free(date);
            free(time);
            free(text);
            return dt2;
...
char *unfold(char *line1, char *line2)
{
    ...
    strcat(line1, line2+trimIndex(line2));
    return line1;
}   
...
VCardErrorCode validateProp(const void* p)
{
    ...
    for(Node* n = prop->parameters->head; n != NULL; n = n->next)
    {
        if(validateParameter(n->data) != OK) {
            printf("2");
...
char* paramToJSON(Parameter* p){
    ...
    char paramString[1000] = "";
    char pValue[100] = "";

    strcpy(pValue, p->value);

    if(p->value[0] == '\"' || p->value[strlen(p->value) - 1] == '\"'){
        memset (pValue, 0, 100);
        strncpy(pValue, p->value+1, strlen(p->value) - 2);
    } 

    sprintf(paramString, "{ \"name\": \"%s\" , \"value\": \"%s\" }", p->name, pValue);

    char* toReturn = malloc(sizeof(char)*strlen(paramString) + 1);
    strcpy(toReturn, paramString);
...
char* propListToJSON(List* props){
    ...
    for(Node *node = props->head; node != NULL; node = node->next){
        Property *prop = node->data;
    ...
    char* toReturn = malloc(sizeof(char) * strlen(json) + 1);

    strcpy(toReturn, json);
...
char* cardToJSON(Card* obj){
    ...
    char* fn = propToJSON(obj->fn);
    char* list = propListToJSON(obj->optionalProperties);
    char* birth = dtToJSON(obj->birthday);
    char* anni = dtToJSON(obj->anniversary);

    char output[3500] = "";

    sprintf(output, "{ \"FN\" : %s , \"list\": %s , \"birth\": %s , \"anni\": %s }", fn, list, birth, anni);
    free(fn); free(list); free(birth); free(anni);

    char* toReturn = malloc(sizeof(char)*strlen(output) + 1);
    strcpy(toReturn, output);
...
char* fileToJSON(char* filename){
    Card* c = NULL;
    int status = 0;
    status = createCard(filename, &c);
    if(status != 0) return errorToString(status);
    status = validateCard(c);
    if(status != 0) return errorToString(status);
    char* output = cardToJSON(c);
    deleteCard(c);
    return output;
}
...
char* addPropToFile(char* filename, char* propJSON){
    ...
    status = createCard(filename, &c);
    if(status != 0) return errorToString(status);
    Property* toBeAdded = JSONtoProp(propJSON);
    addProperty(c, toBeAdded);
    status = validateCard(c);
    if(status != 0) return errorToString(status);
    writeCard(filename, c);
    deleteCard(c);
    return errorToString(status);
}

//{"group":"","name":"FN","values":["Simon Perreault"]}
//[ {"group":"","name":"FN","values":["Simon  Perreault"]} , {"group":"","name":"FN","values":["Simon Perreault"]} , {"group":"","name":"FN","values":["Simon  Perreault"]} ]
...
char* createCardFromJSON(char* fileName, char* json, char* props, char* birth, char* anni){
    ...
    if(strlen(birth) > 2){
        DateTime* birthObj = JSONtoDT(birth);
        c->birthday = birthObj;
    }

    if(strlen(anni) > 2){
        DateTime* anniObj = JSONtoDT(anni);
        c->anniversary = anniObj;
    }

    for(int i = 0; i < strlen(props); i++){
        if(props[i] == '{' && state == 0){
            state = 1;
            curObj[count] = props[i];
            count++;
        } 
        else if(state == 1){
            curObj[count] = props[i];
            ...
            if(props[i] == '}'){
                ...
                state = 0;
                // parse and add prop
                Property* newProp = JSONtoProp(curObj);
                addProperty(c, newProp);
                memset(curObj, 0, 1000);
            }
            if(props[i] == '\"'){
                state = 2;
            }
    ...
    VCardErrorCode result = validateCard(c);

    if(result == OK){
        result = writeCard(fileName, c);
    }
...
char* updateValue(char* fileName, char* currentJSON, char* newJSON){
    ...
    Property* curProp = JSONtoProp(currentJSON);
    Property* newProp = JSONtoProp(newJSON);


    //name change
    if(strcmp(curProp->name, "FN") == 0){
     	char* curVal = c->fn->values->head->data;
    	c->fn->values->head->data = malloc(sizeof(newProp->values->head->data));
    	strcpy(c->fn->values->head->data, newProp->values->head->data);
    	free(curVal);
    } 
    //Other change
    else {
        for (Node *node = c->optionalProperties->head; node != NULL; node = node->next)
        {
            Property *OrigProperty = node->data;
            
            if(strcmp(OrigProperty->name, curProp->name) == 0){
                char* curVal = curProp->values->head->data;
                char* compVal = OrigProperty->values->head->data;
                if(strcmp(curVal, compVal) == 0){
                    //deleteProperty(node->data);
                    char* oldVal = OrigProperty->values->head->data;
                    OrigProperty->values->head->data = malloc(sizeof(newProp->values->head->data));
                    strcpy(OrigProperty->values->head->data, newProp->values->head->data);
                    free(oldVal);
                    break;
                }
            }
    ...
    if(status == OK){
        status = writeCard(fileName, c);
    }

    deleteCard(c);
    deleteProperty(curProp);
