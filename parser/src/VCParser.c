...
VCardErrorCode createCard(char *fileName, Card **newCardObject)
{
    ...
    char *fileContent = readFile(fileName, &result);
    ...
    char beginning[11];
    strncpy(beginning, fileContent, 11);
    if(strcmp(beginning, "BEGIN:VCARD") != 0){
        deleteCard(card);
        free(fileContent);
        return INV_CARD;
    } 
    ...
    for (Node *n = folded->head; n != NULL; n = n->next)
    {
        ...
        if(n->next == NULL && strcmp(name, "END") != 0){
            freeList(folded);
            deleteCard(card);
            free(name);
            return INV_CARD;
        }
        else if (strcmp(name, "END") == 0 && n->next != NULL){
        ...
        else if (strcmp(name, "BEGIN") == 0 || strcmp(name, "VERSION") == 0 || strcmp(name, "END") == 0)
        {
        }
...
char *errorToString(VCardErrorCode err)
{
    ...
    switch(err){
        ...
        break;
        case INV_CARD:
        strcpy(msg,"Invalid card");
        break;        
        case INV_PROP:
        strcpy(msg,"Invalid property");
        break;
        case INV_DT:
        strcpy(msg,"Invalid Date time");
        break;
        case WRITE_ERROR:
    ...
    char* toReturn = malloc(sizeof(char)*strlen(msg) + 1);
    strcpy(toReturn, msg);
    return toReturn;
}
// *************************************************************************
...
char *propToJSON(const Property *prop)
{
    ...
    strcat(json, "\"parameters\": [");
    for(Node *node = p->parameters->head; node != NULL; node = node->next){
        Parameter *param = node->data;
        char* paramJSON = paramToJSON(param);
        strcat(json, paramJSON);
        free(paramJSON);
        if(node->next != NULL){
            strcat(json, ",");
        }
...
Property *JSONtoProp(const char *str)
{
    ...
    int idx = getIndex(g, ':');
    char group[20] = "";
    strncpy(group, g+idx+2, strlen(g));
    //parse the name
    idx = getIndex(n, ':');

    char name[20] = "";
    //char *name = stringCopy(n, idx st+ 2, strlen(n));
    strncpy(name, n+idx+2, strlen(n));
...
Card *JSONtoCard(const char *str)
{
    ...
    char* temp2 = getValuefromJSON(s);

    sprintf(temp, "FN:%s", temp2);

    free(temp2);

    card->fn = parseProperty(temp);
...
VCardErrorCode validateCard(const Card *obj)
{
    ...
    if (revCount > 1 || prIdCount > 1 || genCount > 1 || nCount > 1 || kCount > 1 || uIdCount > 1){
        return INV_PROP;
    }
